...
vec2 calculateUV(vec2 uv, float imgWidth, float imgHeight) {
  ...
  if (ubuf.fillMode < 0.5) {
    ...
    vec2 imageOffset = (vec2(ubuf.screenWidth, ubuf.screenHeight) -
                        vec2(imgWidth, imgHeight)) *
                       0.5;
    ...
    float scale =
        max(ubuf.screenWidth / imgWidth, ubuf.screenHeight / imgHeight);
    vec2 scaledImageSize = vec2(imgWidth, imgHeight) * scale;
    vec2 offset =
        (scaledImageSize - vec2(ubuf.screenWidth, ubuf.screenHeight)) /
        scaledImageSize;
    transformedUV = uv * (vec2(1.0) - offset) + offset * 0.5;
  } else if (ubuf.fillMode < 2.5) {
    // Mode 2: fit (contain) - Fit inside screen, maintain aspect ratio
    float scale =
        min(ubuf.screenWidth / imgWidth, ubuf.screenHeight / imgHeight);
    vec2 scaledImageSize = vec2(imgWidth, imgHeight) * scale;
    vec2 offset =
        (vec2(ubuf.screenWidth, ubuf.screenHeight) - scaledImageSize) * 0.5;
...
vec4 sampleWithFillMode(sampler2D tex, vec2 uv, float imgWidth,
                        float imgHeight) {
  vec2 transformedUV = calculateUV(uv, imgWidth, imgHeight);

  // Check if UV is out of bounds
  if (transformedUV.x < 0.0 || transformedUV.x > 1.0 || transformedUV.y < 0.0 ||
      transformedUV.y > 1.0) {
...
void main() {
  ...
  vec4 color1 =
      sampleWithFillMode(source1, uv, ubuf.imageWidth1, ubuf.imageHeight1);
  vec4 color2 =
      sampleWithFillMode(source2, uv, ubuf.imageWidth2, ubuf.imageHeight2);
  ...
  stripeProgress =
      ...
      (3.0 - 2.0 * stripeProgress); // Smootherstep instead of smoothstep
  ...
  if (isOddStripe) {
    ...
    edgePosition =
        maxPerp + margin - stripeProgress * (perpRange + margin * 2.0);
  } else {
    // Even stripes: edge moves from min to max
    edgePosition =
        minPerp - margin + stripeProgress * (perpRange + margin * 2.0);
  }
  ...
  if (isOddStripe) {
    ...
    mask =
        smoothstep(edgePosition - edgeSmooth, edgePosition + edgeSmooth, yPos);
  } else {
    // Even stripes reveal new wallpaper from top
    mask = 1.0 - smoothstep(edgePosition - edgeSmooth,
                            edgePosition + edgeSmooth, yPos);
  }
  ...
  if (ubuf.progress <= 0.0) {
    ...
    float vignette =
        1.0 - ubuf.progress * 0.1 * (1.0 - abs(stripeProgress - 0.5) * 2.0);
