...
NPanel {
  ...
  property string presetAction: "" // Action to start immediately when panel opens
  ...
  // Lifecycle handlers
  onOpened: {
    // If we have a preset action, select it and start timer
    if (presetAction !== "") {
      // Find the index of preset action
      for (var i = 0; i < powerOptions.length; i++) {
        if (powerOptions[i].action === presetAction) {
          selectedIndex = i
          break
        }
      }
      // Start the countdown timer
      startTimer(presetAction)
      // Clear preset for next time
      presetAction = ""
    } else {
      // Normal open - reset to first option
      selectedIndex = 0
    }
  }
