...
Singleton {
  ...
  function init() {
    ...
    for (var i = 0; i < monitors.length; i++) {
      if (monitors[i].name && monitors[i].wallpaper) {
        currentWallpapers[monitors[i].name] = Settings.preprocessPath(monitors[i].wallpaper)
      }
    }
  ...
  function getWallpaper(screenName) {
    return currentWallpapers[screenName] || Settings.preprocessPath(Settings.data.wallpaper.defaultWallpaper)
  }
