...
-- early return if claude is not available
if not is_claude_available() then
	return
end
-- define module
local M = {}
...
function M._start_claude_terminal(terminal_buf, terminal_win, command)
	vim.api.nvim_set_current_win(terminal_win)
	-- construct claude command
	local claude_cmd = command and string.format("claude %s", command) or "claude"
	-- start terminal
...
-- send prompt from buffer to claude terminal
-- @param prompt_buf: number - prompt buffer number
function M._send_prompt_from_buffer(prompt_buf)
	...
	M._send_to_claude_terminal(table.concat(prompt_lines, "\n"))
end

-- send prompt to claude terminal
-- @param prompt: string - prompt text
function M._send_to_claude_terminal(prompt)
	if not M._windows.claude_job_id then
		vim.notify("Claude terminal not started", vim.log.levels.ERROR)
		return
	end
	-- send prompt to terminal
	vim.fn.chansend(M._windows.claude_job_id, prompt)
...
-- hide the layout but keep claude process running
...
-- show the layout with existing claude process
...
-- completely close the layout and terminate claude process
function M.close_layout()
	-- terminate claude process
	if M._windows.claude_job_id then
		vim.fn.jobstop(M._windows.claude_job_id)
		vim.notify("Claude process terminated", vim.log.levels.INFO)
	end
	M._close_windows()
...
-- open new claude code layout
function M.open_layout()
	local layout = M._create_split_layout()
	if not M._start_claude_terminal(layout.terminal_buf, layout.terminal_win) then
		M.close_layout()
		return
	end
...
-- resume claude session with -r flag
function M.resume_session()
	...
	local layout = M._create_split_layout()
	-- start with -r flag
	if not M._start_claude_terminal(layout.terminal_buf, layout.terminal_win, "-r") then
		M.close_layout()
		return
	end
	vim.api.nvim_set_current_win(layout.prompt_win)
...
-- setup commands
function M.setup()
	vim.api.nvim_create_user_command("ClaudeCode", M.toggle_layout, {
		desc = "Toggle Claude Code terminal and prompt layout",
	})
end
