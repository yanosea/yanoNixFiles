-- kiro cli integration config
-- check if kiro-cli command is available
local function is_kiro_available()
	local handle = io.popen("command -v kiro-cli 2>/dev/null")
	if handle then
...
-- early return if kiro-cli is not available
if not is_kiro_available() then
	return
end
-- define module
local M = {}
...
function M._recreate_split_layout()
	...
	vim.api.nvim_win_set_option(right_win, "winbar", "  KIRO")
	-- create horizontal split for prompt
	vim.cmd("split")
	local prompt_win = vim.api.nvim_get_current_win()
	M._windows.prompt_win = prompt_win
...
function M._setup_terminal_keymaps(terminal_buf, terminal_win)
	-- set winbar for kiro terminal
	vim.api.nvim_win_set_option(terminal_win, "winbar", "  KIRO")
...
function M._start_kiro_terminal(terminal_buf, terminal_win, command)
	...
	-- construct kiro command
	local kiro_cmd = command and string.format("kiro-cli %s", command) or "kiro-cli"
	-- start terminal
	local job_id = vim.fn.termopen(kiro_cmd, {
		buffer = terminal_buf,
		on_exit = function()
			vim.notify("Kiro terminal closed", vim.log.levels.INFO)
	...
	if job_id == 0 then
		vim.notify("Failed to start kiro-cli", vim.log.levels.ERROR)
		return false
	end

	M._windows.kiro_job_id = job_id
	M._setup_terminal_keymaps(terminal_buf, terminal_win)
	return true
end

-- send prompt from buffer to kiro terminal
-- @param prompt_buf: number - prompt buffer number
function M._send_prompt_from_buffer(prompt_buf)
	...
	M._send_to_kiro_terminal(table.concat(prompt_lines, "\n"))
end

-- send prompt to kiro terminal
-- @param prompt: string - prompt text
function M._send_to_kiro_terminal(prompt)
	if not M._windows.kiro_job_id then
		vim.notify("Kiro terminal not started", vim.log.levels.ERROR)
		return
	end
	-- send prompt to terminal
	vim.fn.chansend(M._windows.kiro_job_id, prompt)
...
-- hide the layout but keep kiro process running
...
-- show the layout with existing kiro process
...
-- completely close the layout and terminate kiro process
function M.close_layout()
	-- terminate kiro process
	if M._windows.kiro_job_id then
		vim.fn.jobstop(M._windows.kiro_job_id)
		vim.notify("Kiro process terminated", vim.log.levels.INFO)
	end
	M._close_windows()
...
-- open new kiro layout
function M.open_layout()
	local layout = M._create_split_layout()
	if not M._start_kiro_terminal(layout.terminal_buf, layout.terminal_win) then
		M.close_layout()
		return
	end
...
-- setup commands
function M.setup()
	vim.api.nvim_create_user_command("Kiro", M.toggle_layout, {
		desc = "Toggle Kiro terminal and prompt layout",
	})
end
